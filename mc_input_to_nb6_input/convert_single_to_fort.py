...
target_file = open('./fort.10_single', 'w')
input_file = open('./single_nbody.dat', 'r')
target_file_masses_sorted = open('./fort10_mass_sorted.dat', 'w')

Astrophysical_masses,x,y,z,vx,vy,vz=np.loadtxt(input_file,unpack=True,usecols=(0,1,2,3,4,5,6))
masses_sum = 51773.830000002366
print "Total mass of stars =", masses_sum
print "Total mass of single stars =", np.sum(Astrophysical_masses)
#data = np.loadtxt(input_file,unpack=True,usecols=(0,1,2,3,4,5,6))
#print type(data)
#masses_sum = float(sys.argv[1])
print "Total mass of binary stars =", masses_sum - np.sum(Astrophysical_masses)
#print masses_sum
#Astrophysical_masses = data[0,:]
#print data[0,:]

masses = Astrophysical_masses / masses_sum
#print type(x)
np.savetxt(target_file, np.transpose([masses,x,y,z,vx,vy,vz]),fmt='%20.16f',delimiter=' ')

#print Astrophysical_masses
#print masses
#newline = os.linesep # Defines the newline based on your OS.

#data = Astrophysical_masses,x,y,z,vx,vy,vz
#data_masses_sorted = np.zeros(data.size)
ind=np.argsort((masses))

print 'N =', Astrophysical_masses.size
print 'M = ', masses_sum 
print 'M mean =', masses_sum / 105000.
aa = Astrophysical_masses.size
...
#target_file.write(Astrophysical_masses,x,y,z,vx,vy,vz + newline)
#target_file.close()
